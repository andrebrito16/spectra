...
# Canary: Release-optimised build of the current checkout installed as
# "/Applications/Spectra Canary.app" (own bundle id, yellow icon, own
# Application Support dir, no Sparkle) so it runs next to the stable app.
scripts/canary.sh
```

The `Canary` build configuration (cloned from Release in `project.pbxproj`) sets
`SWIFT_ACTIVE_COMPILATION_CONDITIONS = CANARY`; everything flavour-specific goes
through `AppInfo` (`spectra/App/AppInfo.swift`) — never hardcode "Spectra" for a
path or bundle id. The yellow icon is generated by `scripts/make-canary-icon.swift`.
